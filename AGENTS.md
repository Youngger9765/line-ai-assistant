...
- Storage: `vercel integration add neon` → choose **Neon (Serverless Postgres)** from the interactive list. Neon provisions a free Postgres DB and auto-injects `POSTGRES_URL` (the storage layer in `lib/redis.js` reads it — no code change; a bare/wrong slug errors with "Product not found"). **First run only:** the CLI prints a marketplace-terms URL (a `verification_uri`) that must be accepted once in a browser — surface that URL (and any code) to the user exactly like the `vercel login` link above and **wait** for them to accept before continuing; headless cannot pass it (observed 2026-07-15). Do NOT use Upstash — its Vercel integration has no free tier.
